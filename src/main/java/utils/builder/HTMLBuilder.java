...
public class HTMLBuilder {
  ...
  public String buildHTML(CBRCase _case) {
    ...
    return html(buildHead(fDescription.getFormName()), buildBody(fDescription, fSolution)).render();
  }
      ...
      case "select":
        return div(label(elementName),
            select(option("default 1"), option("default 2"), option("default 3"))
                .withClass("form-control").withName(elementName)).withClass("form-group");
      ...
      for (int i = 0; i < elementName.length(); i++) {
        switch (elementName.charAt(i)) {
          ...
          default:
            ...
            if (nextClosingGroup == -1 && nextClosingCol == -1 && nextClosingRow == -1) {
              ...
              nextChar = Math.min(nextClosingGroup, Math.min(nextClosingCol, nextClosingRow));
            }
            String word = elementName.substring(i, nextChar);
            String HTMLelement = buildFormElement(word, labels.get(word).getLabel()).render();
